...
每日自动预约按用户开启时间先到先得，每个执行日最多创建 10 个自动任务。关闭每日开关会取消尚未启动的每日任务；已经运行或已经向学校提交的任务不会撤销。网页“启动预约”创建立即运行任务，由调度器下一轮认领，持续重新查询当前候选，直至成功、明确异常或用户停止；它不等待次日 07:27，也不受每日任务 07:33 截止时间约束。普通立即运行轮次使用约 1 秒间隔，明确限流沿用安全退避。停止期间若最终提交结果无法确认，任务记录为“结果待核对”，同一目标日不会再次自动提交。
