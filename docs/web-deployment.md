...
Web 版是桌面程序之外的可选部署方式。浏览器负责配置和查看结果，独立调度器在服务器上运行原有 `jlu_booking.auto` 预约核心。生产环境只从已提交的 Git 修订部署；Token、同行人、数据库、密钥、备份和运行日志全部留在服务器数据目录，不进入仓库。
...
Web 与调度器必须是两个独立服务。调度器启动预约子进程；Web 请求进程不直接启动、停止或等待预约核心。
...
访问 HTTPS 页面，检查证书、注册、登录、退出、CSRF 拒绝和移动端布局。公网只经过 Caddy；不要让 8000 端口监听公网地址。
