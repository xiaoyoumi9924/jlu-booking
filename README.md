...
项目同时提供面向小规模用户的可选服务器网页：公开注册的用户分别绑定自己的 JLU Token 和同行人。登录后可在参考桌面 GUI 的工作台实时查询场地、明确确认手动预约，或使用“启动预约”立即开始持续尝试（成功、明确异常或手动停止后结束）；“自动预约”则设置每天 07:27 启动的任务。管理员登录后进入独立管理后台。网页复用原有预约核心；桌面 GUI、命令行和本地定时方式继续保留。
