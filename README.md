...
自动任务内置七档时间优先级；同一时间段优先选择你设置的场地。07:29:55
进入核心阶段后会重新读取最新场地状态，检测到服务器开放后，每个候选明确失败都会
立即刷新并尝试当前最优的下一个候选，避免反复卡在已经被占用的场地。核心阶段到
07:33 结束，收尾阶段到 07:36 结束；全天捡漏已关闭，之后当天不再运行。详细规则见
